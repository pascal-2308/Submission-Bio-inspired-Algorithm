function next_state = physics_quadrocopter(current_state, actions, dt)
...
state = current_state;
for k = 1:n_sub
    state = step_dynamics(state, motor_thrusts, dt_sub);
end
next_state = state;

end

function next_state = step_dynamics(current_state, actions, dt)


% this function is assuming that the quadcopter experiences no turbulence
% and no noise. Drag is added as a stabilizer 
...
params.drag = 0.15; %adding a linear drag coefficient to counteract high velocities
...
% Translational Acceleration EOM over Newtons formula F = m*a 
...
% Rotational Acceleration EOM over Eulers Motion
...
% Define state vector and outputs
...
% prevent the dynamics to allow for a movement through the ground
if next_state(3) <0
    next_state(3) = 0;
